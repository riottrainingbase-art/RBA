...
const fields:Record<Kind,Field[]>={
 person:[{name:"name",label:"記録する人の名前",required:true,max:60,hint:"ニックネームで登録できます。"},{name:"relationship",label:"誰の記録ですか？",options:[["self","自分"],["child","保護者として子どもの記録を残す"]]},{name:"age_group",label:"対象年代・立場",options:ages.map(x=>[x,x==="ADULT"?"一般":x==="COACH"?"指導者":x])},{name:"region",label:"活動する地域",max:80,hint:"都道府県や市区町村まで。住所は不要です。"}],
 history:[{name:"title",label:"参加したクリニック・キャンプ名",required:true,max:160,hint:"例：仙台バスケットボールクリニック"},{name:"occurred_on",label:"参加日",type:"date",required:true},{name:"date_precision",label:"日付の記録方法",options:[["day","参加日を覚えている"],["month","月まで覚えている（日付はその月の1日を選択）"]]},{name:"venue",label:"地域・会場／国・都市",max:160,hint:"例：仙台、石垣島、台北、オンライン。国内外を問わず記録できます。"},{name:"takeaway",label:"覚えていること・学んだこと",type:"textarea",max:2000},{name:"next_action",label:"今の練習で試したいこと",type:"textarea",max:1000}],
 ...
 goal:[{name:"horizon",label:"どのくらい先を見ますか？",options:[["week","今週の一歩"],["three_months","3か月の挑戦"],["vision","これからのビジョン"]]},{name:"title",label:"どんな自分になりたいですか？",required:true,max:160},{name:"action",label:"そのために取り組むこと",type:"textarea",required:true,max:1000},{name:"success",label:"できたと分かる目印",type:"textarea",required:true,max:1000,hint:"結果だけでなく、行動でも決められます。例：練習ごとに一度、自分から声をかける。"},{name:"target_on",label:"振り返る予定日",type:"date",required:true},{name:"status",label:"取り組み状況",options:[["active","取り組み中"],["achieved","達成した"],["paused","いったん休む"]]}]
};
const titles:Record<Kind,string>={person:"記録する人を登録",history:"これまでの参加・経験を記録",checkin:"今の自分を振り返る",goal:"次の目標を決める"};
const horizons:Record<string,string>={week:"今週の一歩",three_months:"3か月の挑戦",vision:"これからのビジョン"};
...
export function BasketballPassport({userId}:{userId:string}){
 ...
 return <section className={styles.passport} aria-labelledby="passport-title">
  <header className={styles.hero}><p>GLOBAL BASKETBALL PASSPORT</p><h2 id="passport-title">これまでの経験を、<br/>次の成長につなげる。</h2><p>地域での練習、全国のクリニック、海外交流、オンラインでの学びまで。どこで何を経験し、何を学び、次に何を試したいのかを、自分の成長記録として残せます。</p></header>
  ...
  {loading?<p role="status">記録を読み込んでいます…</p>:failed?<button onClick={()=>void load(personId)}>再読み込み</button>:<>
   ...
   {person&&<>
    ...
    <nav className={styles.tabs} aria-label="成長記録">{([["overview","全体を見る"],["history","参加履歴"],["checkin","振り返り"],["goal","これから"],["media","写真・動画"]] as const).map(([key,label])=><button key={key} aria-pressed={view===key} disabled={!!form||mediaLocked} onClick={()=>{setView(key);setLimit(10);setRemoving(null);}}>{label}</button>)}</nav>
    ...
    {view==="overview"&&<>
     <article className={styles.panel}><h3>記録を残して、自分の変化を見つける。</h3><p>写真や動画を以前の自分と見比べ、気づいたことを次の練習につなげます。</p><button onClick={()=>setView("media")}>写真・動画の成長ノートを開く</button></article>
     <div className={styles.metrics}><article><span>自分で残した参加履歴</span><strong>{history.length}<small>件</small></strong></article><article><span>自分の振り返り</span><strong>{checkins.length}<small>回</small></strong></article><article><span>達成した目標</span><strong>{goals.filter(g=>g.status==="achieved").length}<small>件</small></strong></article></div>
     <div className={styles.steps}>{([['history','01','参加した経験を残す','参加した活動と、そこで学んだことを記録。'],['checkin','02','今の自分を知る','できたこと、課題、次の一歩を整理。'],['goal','03','次の目標を決める','今週取り組むことから、将来の目標まで。']] as const).map(([kind,num,title,body])=><button key={kind} onClick={()=>open(kind)}><span>{num}</span><strong>{title}</strong><p>{body}</p></button>)}</div>
     <div className={styles.columns}><article className={styles.panel}><span>振り返り · {latest?dateLabel(latest.checked_on):"まだ記録がありません"}</span><h3>今の自分</h3>{latest?<><h4>できるようになったこと</h4><p>{latest.strengths}</p><h4>取り組みたい課題</h4><p>{latest.challenge}</p><h4>次の練習で試すこと</h4><p>{latest.next_action}</p></>:<p>他の人との比較ではなく、自分の変化を言葉にしてみましょう。</p>}<button onClick={()=>open("checkin")}>今の自分を記録する</button></article><article className={styles.panel}><span>これから</span><h3>次に挑戦すること</h3>{activeGoals.length?activeGoals.slice(0,3).map(g=><div className={styles.goalSummary} key={g.id}><small>{horizons[g.horizon||""]} · {dateLabel(g.target_on)}</small><h4>{g.title}</h4><p>{g.action}</p></div>):<p>大きな夢でも、小さな一歩でも。自分で目標を決めてみましょう。</p>}<button onClick={()=>open("goal")}>目標を追加</button></article></div>
    </>}
  ...
  {person&&!failed&&!loading&&<footer className={styles.footer}><div><h3>次の挑戦へ</h3><p>{person.age_group==="COACH"?"国内外の指導者向け学習から、次の学びを選べます。":"地域や年代から次の機会を探し、興味があれば海外やオンラインの機会にも視野を広げられます。"}</p><a href={`/ja/opportunities?${recommend}`}>自分に合う育成機会を探す →</a><a href="/ja/international">世界につながる活動を見る →</a><a href={person.age_group==="COACH"?"/ja/d-hub":"/ja/players"}>{person.age_group==="COACH"?"D-HUBで学びを続ける":"日々の練習のヒントを見る"} →</a></div><div><button onClick={exportRecords}>記録をダウンロード</button><p>参加記録の照合については、参加時期・会場を添えて<a href="/ja/contact">RBAへご相談ください。</a></p></div></footer>}
 </section>;
}
