...
export function LocalizedPayments({locale}:{locale:Locale}){
  ...
  return <SiteFrame locale={locale} languagePage="payments">
    ...
    <section className="homecourt-payment section-pad"><div><p className="section-index">RBA ID → MY HOME COURT</p><h2>{locale==="ja"?"次の挑戦につながる、自分だけのホームコート。":"Your next opportunity starts here."}</h2><p>{locale==="ja"?"活動情報、参加履歴、立場に合った育成コンテンツをひとつに。クリニックで得た経験を、次の一歩へつなげます。":"Discover programmes, community routes and learning for players, families and coaches."}</p><a className="button button-member" href={homecourtFreeRegistrationUrl(locale)}>{locale==="ja"?"RBA IDで始める":"Explore RBA ID"}<ArrowRight size={16}/></a></div><div><span>{locale==="ja"?"RBA HOMECOURT メンバーシップ":"RBA HOMECOURT monthly"}</span><strong>{formatJPY(HOMECOURT_PRICE_JPY)}</strong><small>{locale==="ja"?"ログイン後、参加する内容と金額をStripe画面で確認してからお申し込みください。":"Contact RBA to confirm the membership details and payment arrangements."}</small><a className="button button-orange" href={homecourtFreeRegistrationUrl(locale)}><CreditCard size={17}/>{locale==="ja"?"ログインして詳細を見る":locale==="zh-tw"?"登入後加入":locale==="ko"?"로그인 후 가입":"Sign in to join"}<ArrowRight size={16}/></a><a className="text-link" href={localePath(locale,"home-court")}>{locale==="ja"?"MY HOME COURTを見る":"Compare membership"}<ArrowRight size={16}/></a></div></section>
    <section className="payment-event-list section-pad">{ids.filter(id=>!programmeById[id].registrationClosed).map(id=>{const p=programmeById[id];const options=eventPayments[id];return <article id={id} key={id}><div className="payment-event-head"><div><time>{p.date[locale==="en"?0:locale==="ja"?1:locale==="zh-tw"?2:3]}</time><h2>{p.title[locale==="en"?0:locale==="ja"?1:locale==="zh-tw"?2:3]}</h2><p>{p.place[locale==="en"?0:locale==="ja"?1:locale==="zh-tw"?2:3]}</p></div><a className="button button-dark" href={p.applicationUrl} target="_blank" rel="noreferrer"><FileText size={17}/>{c.openForm}<ArrowUpRight size={16}/></a></div><div className="payment-options"><p className="section-index">{c.pay}</p>{options.length?options.map(option=><a href={onlineCheckoutReady?`/api/commerce/checkout/${option.id}`:localePath(locale,"contact")} key={option.id}><span>{option.label}</span><strong>{formatJPY(option.amount)}<ArrowUpRight size={17}/></strong></a>):<p className="payment-pending">{c.pending}</p>}</div></article>})}</section>
