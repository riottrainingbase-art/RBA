...
const copy = {
  ...
  ja:{kicker:"公式申込・決済",title:"お申し込み後、案内に沿って安全にお支払いいただけます。",intro:"参加する企画とプランを確認し、対応する申込フォームとStripeの決済ページをご利用ください。申込時と決済時は、同じメールアドレスをご入力ください。",apply:"1. 参加申込",pay:"2. Stripe決済",openForm:"申込フォームを開く",openPay:"Stripe決済ページを開く",pending:"参加費と決済方法は、まだ公開されていません。まず申込フォームを送信し、RBAから参加費の案内が届いてからお支払いください。",notice:"決済前に必ずご確認ください",points:["参加者1名につき、申込フォームを1件送信してください。","Stripe画面に表示される企画名、プラン、金額を確認してから決済してください。","決済完了、またはRBAからの受付完了の案内をもって参加が確定します。","キャンセル・返金条件はRBA共通規定をご確認ください。個別イベントに別条件がある場合は、申込前に表示された個別条件を優先します。"],complete:"決済後にStripeから届く支払い完了メールは、お申し込み内容の確認に必要となる場合がありますので保管してください。"},
...
export function LocalizedPayments({locale}:{locale:Locale}){
  ...
  const paused={
    ja:{title:"お申し込み・お支払いのご案内",intro:"各企画の申込フォームをご利用ください。現在、お支払い方法はRBAが個別にご案内しています。申込済みの方は、企画名と参加プランを添えてお問い合わせください。",pay:"お支払い方法を確認する"},
  ...
  return <SiteFrame locale={locale} languagePage="payments">
    ...
    <section className="homecourt-payment section-pad"><div><p className="section-index">RBA ID → MY HOME COURT</p><h2>{locale==="ja"?"申し込みから、参加後の成長まで。":"Your next opportunity starts here."}</h2><p>{locale==="ja"?"申込時と同じメールアドレスでRBA IDを作成すると、活動情報、参加履歴、Basketball Passport、立場に合った学びを一つにつなげられます。クリニックで得た経験も、次の挑戦へつなげて残せます。":"Discover programmes, community routes and learning for players, families and coaches."}</p><a className="button button-member" href={homecourtFreeRegistrationUrl(locale)}>{locale==="ja"?"RBA IDで始める":"Explore RBA ID"}<ArrowRight size={16}/></a></div><div><span>{locale==="ja"?"HOMECOURT PLUS":"RBA HOMECOURT monthly"}</span><strong>{formatJPY(HOMECOURT_PRICE_JPY)}</strong><small>{locale==="ja"?(onlineCheckoutReady?"月額3,300円。内容をご確認のうえ、そのままStripeでお手続きいただけます。":"内容とお申し込み方法をRBAがご案内します。"):"Contact RBA to confirm the membership details and payment arrangements."}</small><a className="button button-orange" href={onlineCheckoutReady?homecourtCheckout:localePath(locale,"contact")}><CreditCard size={17}/>{onlineCheckoutReady?(locale==="ja"?"HOMECOURT PLUSを始める":locale==="zh-tw"?"開始月費會員方案":locale==="ko"?"월간 멤버십 시작":"Start monthly membership"):(locale==="ja"?"メンバーシップについて相談する":locale==="zh-tw"?"諮詢會員方案":locale==="ko"?"멤버십 문의":"Ask about membership")}<ArrowRight size={16}/></a><a className="text-link" href={HOMECOURT_BILLING_PORTAL} target="_blank" rel="noopener noreferrer">{({ja:"契約中の方：カード変更・請求書・解約",en:"Members: card, invoices & cancellation","zh-tw":"已訂閱：信用卡、帳單與取消",ko:"구독 중: 카드·청구서·취소"})[locale]}<ArrowUpRight size={16}/></a><a className="text-link" href={localePath(locale,"home-court")}>{locale==="ja"?"MY HOME COURTを見る":"Compare membership"}<ArrowRight size={16}/></a></div></section>
    {locale==="ja"?<section className="statement section-pad"><p className="section-index">AFTER APPLICATION</p><div><h2>申し込み後は、RBA IDで次につなげる。</h2><p>申込時と同じメールアドレスでRBA IDを作成すると、今回の参加をMY HOME COURTにつなげられます。参加予定、過去の経験、次の活動まで、一回のクリニックで終わらせないための入口です。</p><a className="text-link" href="/ja/after-application">申込後の使い方を見る<ArrowRight size={16}/></a></div></section>:null}
    ...
    <section className="next-page section-pad"><p>{c.apply} → {c.pay}</p><a href={`${localePath(locale,"policies")}${locale==="ja"?"#cancellation":""}`}>{locale==="ja"?"参加規約・キャンセル・返金方針を確認":"Terms and cancellation policy"}<ArrowRight size={24}/></a></section>
  </SiteFrame>;
}
