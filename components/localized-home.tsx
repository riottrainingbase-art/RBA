...
const copy={
  ...
  ja:{
    ...
    routesLabel:"育成を、行動に変える",routesTitle:"参加する。学ぶ。挑戦する。つながる。",routesCopy:"理念だけで終わらせず、選手・保護者・指導者それぞれが次の行動に進める入口を用意しています。",routes:[["選手・保護者","クリニック、キャンプ、育成企画を探す","opportunities"],["RBA UNITED","所属を続けながら、大会・遠征・国際交流へ個人参加する","united"],["指導者","トーステン氏のオンライン講習で学ぶ","events/torsten-loibl-online-clinic"],["主催者・団体","地域やクラブでRBAを開催する","clinic-request"],["海外アカデミー","日本との責任ある交流をつくる","asia"]],
  ...
  "zh-tw":{
    ...
    routesLabel:"從這裡開始",routesTitle:"參加。學習。挑戰。合作。",routesCopy:"依照您的目的，直接進入最合適的窗口。",routes:[["球員與家庭","尋找訓練營、活動與培育企劃","opportunities"],["RBA UNITED","維持原球隊，同時以個人身份參加大會、遠征與交流","united"],["教練","參加Torsten Loibl線上教練講座","events/torsten-loibl-online-clinic"],["主辦單位","邀請RBA到您的城市或球隊","clinic-request"],["亞洲夥伴","與日本建立負責任的籃球交流","asia"]],
  ...
  ko:{
    ...
    routesLabel:"여기에서 시작하세요",routesTitle:"참가하고. 배우고. 도전하고. 연결합니다.",routesCopy:"현재 목적에 맞는 창구에서 바로 다음 단계로 이동할 수 있습니다.",routes:[["선수와 가족","클리닉, 캠프와 성장 프로그램 찾기","opportunities"],["RBA UNITED","현재 팀을 유지하면서 대회·원정·교류에 개인 참가하기","united"],["코치","Torsten Loibl 온라인 클리닉에서 배우기","events/torsten-loibl-online-clinic"],["주최자와 단체","지역 또는 팀에 RBA 초대하기","clinic-request"],["아시아 파트너","일본과 책임 있는 농구 교류 만들기","asia"]],
...
export function LocalizedHome({locale}:{locale:Locale}){
  ...
  return <div lang={locale==="zh-tw"?"zh-Hant-TW":locale}><SiteFrame locale={locale}>
    <section className="hero-grid"><div className="hero-copy"><p className="eyebrow">RIOT BASKETBALL ACADEMY · JAPAN</p><h1><span>{c.title[0]}</span><span>{c.title[1]}</span></h1><p className="hero-lede">{c.lede}</p><PlatformQuickFinder locale={locale}/>{locale==="ja"&&<a className="hero-clinic-alert" href={localePath(locale,"events/torsten-loibl-online-clinic")}><span>NOW OPEN</span><strong>{c.clinicHero}</strong><ArrowRight size={18}/></a>}<div className="hero-actions"><a className="button button-light" href={localePath(locale,"opportunities")}>{c.primary}<ArrowRight size={17}/></a><a className="button button-member" href={locale==="en"?"/my-homecourt/login":`/${locale}/my-homecourt/login`}><House size={17}/>{locale==="ja"?"RBA IDを無料でつくる":hc.open}<ArrowRight size={17}/></a><a className="text-link light-link" href={localePath(locale,"clinic-request")}>{c.secondary}<ArrowRight size={16}/></a></div><div className="hero-proof">{c.proof.map(([n,l])=><div key={n}><strong>{n}</strong><span>{l}</span></div>)}</div></div><div className="hero-image" role="img" aria-label={c.image}><div className="image-note">{c.image}</div><MoveDown className="scroll-mark" size={19}/></div></section>
