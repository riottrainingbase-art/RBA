import { CheckCircle2, Mail, ArrowRight, House, History } from "lucide-react";
...
}[locale];const body=membership?membershipCopy.b:c.b;const detail=membership?membershipCopy.d:c.d;const back=membership?membershipCopy.back:c.back;const homecourtHref=locale==="ja"?"/ja/my-homecourt/subscribe":locale==="en"?"/my-homecourt/app":`/${locale}/my-homecourt/app`;return <SiteFrame locale={locale} languagePage="payment-complete"><section className="payment-complete section-pad"><CheckCircle2 size={64}/><p className="section-index">{c.k}</p><h1>{c.t}</h1><p>{body}</p><div><Mail size={22}/><strong>{c.a}</strong><span>{detail}</span></div><a className="button button-orange" href={membership?homecourtHref:localePath(locale,"schedule")}>{back}<ArrowRight size={17}/></a>{locale==="ja"&&!membership?<div className="homecourt-private-note"><House size={24}/><div><strong>今回の参加をMY HOME COURTにつなげる</strong><p>申込時・決済時と同じメールアドレスでRBA IDを作成すると、参加予定や次の育成機会をまとめられます。</p><a className="text-link" href="/ja/after-application">申込後の次の一歩を見る<ArrowRight size={16}/></a></div></div>:null}{locale==="ja"&&!membership?<div className="homecourt-private-note"><History size={24}/><div><strong>過去にRBAへ参加したことがある方へ</strong><p>以前のクリニックやキャンプも、Basketball Passportの最初の記録として残せます。</p><a className="text-link" href="/ja/my-homecourt/participants">過去の参加を記録する<ArrowRight size={16}/></a></div></div>:null}</section></SiteFrame>}
