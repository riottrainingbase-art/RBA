...
const pages:Record<AudienceKind,{
 ...
 sections:{icon:"calendar"|"shield"|"learn"|"home"|"map"|"pay";title:Text4;body:Text4;items:Text4[]}[];
  actions:{label:Text4;href:"schedule"|"contact"|"policies"|"home-court"|"players"|"families"|"coaches"|"events/torsten-loibl-online-clinic"|"clinic-request"|"my-homecourt/login"|"d-hub"|"united"|"camp";external?:string}[];
 homeCourt?:"member"|"partner";
}>={
 players:{
  kicker:["FOR PLAYERS","選手の方へ","給球員","선수를 위해"],
  title:["Find the court that moves you forward.","次の成長につながる場所を見つけよう。","找到讓自己進步的下一個球場。","나를 앞으로 이끄는 다음 코트로."],
  lead:["RBA connects weekly learning, clinics, camps, 3x3 and international experiences. Choose by age, place and the challenge you need now.","毎週のスクールから、全国のクリニック、宿泊キャンプ、3x3、海外交流まで。年代・地域・今の課題から、自分に合う次の機会を選べます。","從每週課程、全國訓練營、住宿營、3x3到海外交流，依年齡、地區與現在的課題選擇。","주간 스쿨부터 전국 클리닉, 숙박 캠프, 3x3, 해외 교류까지 연령과 지역, 현재 과제에 맞춰 선택할 수 있습니다."],
  ...
  sections:[
   ...
   {icon:"home",title:["Another home court","もうひとつのホームコート","另一個主場","또 하나의 홈 코트"],body:["RBA HOMECOURT is the monthly membership for players and families who want a continuing connection to RBA beyond one event.","MY HOME COURTは、一度の参加で終わらせず、その経験を次の活動や学びにつなげるための場所です。","RBA HOMECOURT讓球員與家庭持續參與RBA。","RBA HOMECOURT는 선수와 가족이 RBA와 지속적으로 연결되는 오픈형 멤버십입니다."],items:[["Regional HOMECOURT SESSION","地域のHOMECOURT SESSION","地區SESSION","지역 SESSION"],["Clinics, camps and new member content","クリニック・キャンプと会員向けコンテンツ","活動與會員內容","클리닉·캠프와 회원 콘텐츠"],["A clear next step from MY HOME COURT","MY HOME COURTから次の活動へ","從MY HOME COURT前往下一活動","MY HOME COURT에서 다음 활동으로"]]},
  ],
  actions:[{label:common.schedule,href:"schedule"},{label:["Development Camp","Development Campを見る","查看 Development Camp","Development Camp 보기"] as Text4,href:"camp"},{label:["RBA UNITED","RBA UNITEDを見る","查看 RBA UNITED","RBA UNITED 보기"] as Text4,href:"united"},{label:common.home,href:"home-court"},{label:common.contact,href:"contact"}],homeCourt:"member"
 },
 families:{
  ...
  sections:[
   {icon:"pay",title:["Application and payment","申込・決済","報名與付款","신청·결제"],body:["Submitting a form does not always confirm a place. Follow the payment link shown after submission or the message sent by RBA.","企画によっては、フォームを送信しただけでは申込は確定しません。送信後の確認画面やRBAから届く案内に従って、必要な決済を行ってください。","提交表單後依確認畫面或RBA通知完成付款。","폼 제출 후 확인 화면 또는 RBA 안내에 따라 결제합니다."],items:[["Check what the fee includes","費用に含まれるものを確認","確認費用包含內容","비용 포함 항목 확인"],["Use only the official payment link","公式案内の決済リンクを使用","只使用官方付款連結","공식 결제 링크 사용"],["Keep the confirmation message","受付完了の案内を保管","保存確認通知","확정 안내 보관"]]},
 ...
 coaches:{
  ...
  title:["Learn. Host. Build a home court.","学ぶ。RBAを呼ぶ。地域に新しい育成の場をつくる。","學習、邀請，建立地區主場。","배우고, 초대하고, 지역의 홈 코트를 만듭니다."],
  lead:["Choose the route that fits your role: learn through coach education, invite RBA to your team, or become a HOME COURT PARTNER in your region.","指導者講習で学ぶ、チームや地域でRBAのプログラムを開催する、地域の育成環境づくりに取り組む。今の目的に合うところから始められます。","透過教練教育學習、邀請RBA或成為地區夥伴。","코치 교육, RBA 초청, HOME COURT PARTNER 중 역할에 맞는 길을 선택합니다."],
  statement:["Better environments begin with better questions.","より良い育成環境は、問い直すことから始まる。","更好的培育環境始於更好的問題。","더 나은 육성 환경은 더 나은 질문에서 시작됩니다."],
  sections:[
 ...
 "home-court":{
  ...
  statement:["Not another team. Another place to belong and grow.","新しいチームを増やすのではなく、学びと成長の選択肢を増やす。","不是增加一支隊伍，而是增加可回歸、可成長的地方。","새 팀이 아니라, 돌아와 성장할 수 있는 공간을 늘립니다."],
  sections:[
...
export function AudiencePage({locale,kind}:{locale:Locale;kind:AudienceKind}){
 ...
 return <div lang={locale==="zh-tw"?"zh-Hant-TW":locale}><SiteFrame locale={locale} languagePage={kind}>
  ...
  {p.homeCourt&&<section className="homecourt-bridge section-pad"><div><p className="section-index inverse">{p.homeCourt==="member"?"MY HOME COURT":"HOME COURT PARTNER"}</p><h2>{t(common.home,locale)}</h2><p>{locale==="ja"?(kind==="families"?"チーム選び、出場時間、練習量、移籍。迷ったときに、所属の外にも考える材料と次の選択肢を持てます。":kind==="coaches"?"育成記事、D-HUB、指導者講習、練習設計。指導を一度きりの学びで終わらせず、毎週更新できる場所です。":kind==="players"?"次のクリニック、成長記録、育成記事、全国・海外の機会。自分のバスケを一つの場所につないでいけます。":t(common.notFinal,locale)):t(common.notFinal,locale)}</p></div><div><a className="button button-light" href={audiencePath(locale,kind==="families"?"my-homecourt/login":kind==="coaches"?"my-homecourt/login":"home-court")}>{locale==="ja"?(kind==="families"?"保護者向けMY HOME COURTへ":kind==="coaches"?"指導者向けMY HOME COURTへ":kind==="players"?"選手向けMY HOME COURTへ":t(common.home,locale)):t(common.home,locale)}<ArrowRight size={17}/></a>{locale==="ja"&&kind!=="home-court"?<a className="button button-dark" href="/ja/journal">育成記事を読む<ArrowRight size={17}/></a>:null}</div></section>}
  {kind==="home-court"&&<section className="homecourt-status section-pad"><HeartHandshake size={34}/><div><p className="section-index">{t(["CURRENT STATUS","現在の受付状況","目前狀態","현재 상태"],locale)}</p><h2>{t(["Registration is open","次の一歩を、ここから始める","會員登錄開放中","회원 등록 중"],locale)}</h2><p>{t(common.notFinal,locale)}</p><p>{t(["RBA HOMECOURT connects programmes, participation records and role-based learning. Coaches can also continue to the 48-session D-HUB programme.","MY HOME COURTは、活動、参加記録、立場に合う学びを一つにつなぐ場所です。指導者はD-HUBの継続学習にも進めます。","RBA HOMECOURT整合活動、參與紀錄與角色學習；教練也能繼續參與每年48次的D-HUB。","RBA HOMECOURT는 활동, 참가 기록과 역할별 학습을 연결합니다. 코치는 연간 48회 D-HUB도 이용할 수 있습니다."],locale)}</p></div></section>}
