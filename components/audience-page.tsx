...
const common={
 ...
 home:["RBA HOMECOURT","RBA HOMECOURT","RBA HOMECOURT","RBA HOMECOURT"] as Text4,
  notFinal:["Free registration is open. RBA HOMECOURT is an optional JPY 3,300 monthly plan; the Stripe checkout details take priority.","無料会員登録を受け付けています。RBA HOMECOURTは、希望する方だけが加入する月額3,300円のプランです。申込時は、Stripe決済画面に表示される内容と金額を必ずご確認ください。","免費會員登錄已開放。RBA HOMECOURT為可選的每月3,300日圓方案，以Stripe頁面為準。","무료 회원 등록이 열려 있습니다. RBA HOMECOURT는 선택형 월 3,300엔 플랜이며 Stripe 화면이 우선합니다."] as Text4,
};
...
const pages:Record<AudienceKind,{
 ...
 "home-court":{
  kicker:["RBA HOMECOURT","RBAホームコート","RBA HOMECOURT","RBA HOMECOURT"],
  title:["Another home court.","もうひとつ、ホームコートを。","再多一個主場。","또 하나의 홈 코트를."],
  lead:["An open membership concept connecting local sessions, RBA clinics and camps, activity history and regional partners in one continuing development environment.","地域セッション、全国のクリニックやキャンプ、参加履歴、地域パートナーを、一つの継続的な育成環境としてつなぐ会員サービスです。","把地區SESSION、全國活動、參加紀錄與合作夥伴連成持續的培育環境。","지역 SESSION, 전국 프로그램, 참가 기록과 파트너를 하나의 지속적인 육성 환경으로 연결합니다."],
  ...
  sections:[
   {icon:"home",title:["FREE MEMBER → RBA HOMECOURT","無料会員 → RBA HOMECOURT","免費會員 → RBA HOMECOURT","무료 회원 → RBA HOMECOURT"],body:["Start free for role-based information and community access. Upgrade to the optional monthly plan only when it fits.","無料会員になると、自分の立場に合った情報やコミュニティを利用できます。継続的なサポートや会員特典が必要になったときだけ、月額プランへ移行できます。","先免費加入，需要持續福利時再升級。","무료로 시작하고 지속 혜택이 필요할 때만 업그레이드합니다."],items:[["Free member registration","無料会員登録","免費會員登錄","무료 회원 등록"],["Optional RBA HOMECOURT — JPY 3,300/month","必要に応じて加入できるRBA HOMECOURT・月額3,300円","可選RBA HOMECOURT・每月3,300日圓","선택형 RBA HOMECOURT·월 3,300엔"],["PLAYER / PARENT / COACH routes","PLAYER／PARENT／COACHの3つの入口","PLAYER／PARENT／COACH三入口","PLAYER／PARENT／COACH 경로"]]},
...
export function AudiencePage({locale,kind}:{locale:Locale;kind:AudienceKind}){
 ...
 return <div lang={locale==="zh-tw"?"zh-Hant-TW":locale}><SiteFrame locale={locale} languagePage={kind}>
  ...
  {p.homeCourt&&<section className="homecourt-bridge section-pad"><div><p className="section-index inverse">{p.homeCourt==="member"?"RBA HOMECOURT":"HOME COURT PARTNER"}</p><h2>{t(common.home,locale)}</h2><p>{t(common.notFinal,locale)}</p></div><a className="button button-light" href={audiencePath(locale,"home-court")}>{t(common.home,locale)}<ArrowRight size={17}/></a></section>}
  {kind==="home-court"&&<section className="homecourt-status section-pad"><HeartHandshake size={34}/><div><p className="section-index">{t(["CURRENT STATUS","現在の受付状況","目前狀態","현재 상태"],locale)}</p><h2>{t(["Registration is open","無料会員登録受付中","免費會員登錄開放中","무료 회원 등록 중"],locale)}</h2><p>{t(common.notFinal,locale)}</p><p>{t(["RBA HOMECOURT is RBA’s open membership gateway. D-HUB is a separate, selection-based pathway for competition-focused players.","RBA HOMECOURTは、誰でも利用を始められるRBAの会員サービスです。D-HUBは、競技力の向上を目指す選手を対象とした選考制の別事業です。","RBA HOMECOURT為RBA開放會員入口；D-HUB是獨立選拔方案。","RBA HOMECOURT는 RBA 오픈 회원 입구이며 D-HUB는 별도 선발 사업입니다."],locale)}</p></div></section>}
