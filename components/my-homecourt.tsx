...
export function MyHomecourt({locale,role}:{locale:Locale;role?:HomecourtRole}) {
  ...
  const c=copy[locale];
  const registrationUrl=locale==="en"?"/my-homecourt/app":`/${locale}/my-homecourt/app`;
  const ja=locale==="ja";
  ...
  return <SiteFrame locale={locale} languagePage="my-homecourt">
    ...
    {!role&&ja?<section className="homecourt-product-preview section-pad">
      ...
      <div className="homecourt-preview-grid">
        ...
        <article><Sparkles/><span>FREE</span><h3>RBA IDから始める</h3><p>探す、保存する、次につなげる。まずは無料でHOME COURTをつくる。</p><a className="text-link" href={registrationUrl}>無料で始める <ArrowRight size={16}/></a></article>
      </div>
    </section>:null}
