...
export function OpportunityExplorer({locale}:{locale:Locale}){
  ...
  return <SiteFrame locale={locale} languagePage="opportunities"><div className="opportunity-page">
    ...
    <section className="homecourt-bridge section-pad"><div><p className="section-index inverse">RBA UNITED / INDIVIDUAL ENTRY</p><h2>{({ja:"所属チームを続けながら、個人で次の挑戦へ。",en:"Keep your club. Join the next challenge as an individual.","zh-tw":"保留原球隊，同時以個人身份挑戰下一個機會。",ko:"현재 팀을 유지하면서 개인으로 다음 도전에 참가하세요."})[locale]}</h2><p>{({ja:"RBA UNITEDは、個人で申し込み、大会・遠征・キャンプ・国際交流など企画ごとに期間限定で集まる仕組みです。",en:"RBA UNITED brings individually registered players together for a specific tournament, camp, trip or exchange.","zh-tw":"RBA UNITED讓個人報名球員為特定大會、營隊、遠征或國際交流組成期間限定團隊。",ko:"RBA UNITED는 개인 신청 선수가 대회·캠프·원정·국제교류마다 기간 한정으로 함께하는 프로그램입니다."})[locale]}</p></div><div><a className="button button-light" href={localePath(locale,"united")}>{({ja:"RBA UNITEDとは",en:"How RBA UNITED works","zh-tw":"了解 RBA UNITED",ko:"RBA UNITED 알아보기"})[locale]}<ArrowRight/></a><a className="button button-dark" href="?kind=TRAVEL">{({ja:"遠征・交流を探す",en:"Find travel & exchange","zh-tw":"尋找遠征・交流",ko:"원정·교류 찾기"})[locale]}<ArrowRight/></a></div></section>
    ...
    {locale==="ja"?<section className="homecourt-product-preview section-pad">
      ...
      <div className="homecourt-preview-grid">
        ...
        <article><Bookmark/><span>03</span><h3>Saveする</h3><p>迷ったら保存。あとでMY HOME COURTからもう一度見られます。</p></article>
