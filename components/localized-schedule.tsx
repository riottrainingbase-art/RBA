...
const copy={
  ...
  ja:{kicker:"クリニック・イベント日程",title:["次に参加するRBAの活動を","見つける。"],intro:"クリニック、キャンプ、育成プログラムの開催予定をまとめています。対象年代、参加費、決済方法を確認し、そのまま各イベントの公式申込ページへ進めます。",list:"開催予定",desc:"募集状況、参加費に含まれる内容、キャンセル条件は、リンク先の公式フォームでご確認ください。",target:"対象",payment:"決済方法",apply:"公式申込ページへ",pay:"申込後の決済へ",notice:"新しい開催情報を受け取る",noticeCopy:"RBA公式LINEでは、新しいクリニックや募集開始のお知らせを受け取れます。",line:"公式LINEを開く",back:"RBAトップへ"},
...
export function LocalizedSchedule({locale}:{locale:Locale}){
  ...
  return <div lang={locale==="zh-tw"?"zh-Hant-TW":locale}><SiteFrame locale={locale} languagePage="schedule">
    ...
    <section className="event-list section-pad"><div className="section-head"><div><p className="section-index"><CalendarDays size={15}/> {c.list}</p><h2>{c.list}</h2></div><p>{c.desc}</p></div>{programmes.map((event)=><article key={event.id}><time dateTime={event.startDate}><strong>{tr(event.datePrimary,locale)}</strong><span>{tr(event.dateSecondary,locale)}</span></time><div><p className="note-tag">{tr(event.place,locale)}</p><h3>{tr(event.title,locale)}</h3><p><strong>{c.target}：</strong>{tr(event.audience,locale)}</p><p className="event-price">{tr(event.price,locale)}</p><p className="event-payment"><strong>{c.payment}：</strong>{tr(event.payment,locale)}</p></div><div className="event-actions"><a href={event.applicationUrl} target="_blank" rel="noreferrer">{c.apply}<ArrowUpRight size={17}/></a><a href={`${localePath(locale,"payments")}#${event.id}`}>{c.pay}<ArrowUpRight size={17}/></a></div></article>)}</section>
