...
function load(path, imports={}) {
 ...
 vm.runInNewContext(code,{module:loadedModule,exports:loadedModule.exports,require:key=>{if(key==='server-only')return {};if(key==='react/jsx-runtime')return jsx;if(key in imports)return imports[key];throw Error(key);},Date});
 return loadedModule.exports;
}
const billing=load('lib/homecourt-billing.ts');
const {canReadMemberArticles}=load('lib/member-article-access.ts',{'./homecourt-billing':billing});
...
for(const [patch,expected] of [[{},true],[{status:'trialing'},true],[{cancel_at_period_end:true},true],[{status:'canceled'},false],[{status:'past_due'},false],[{plan_key:'other'},false],[{current_period_end:null},true],[{current_period_end:'invalid'},true],[{cancel_at_period_end:true,current_period_end:null},false],[{cancel_at_period_end:true,current_period_end:'invalid'},false],[{cancel_at_period_end:true,current_period_end:'2026-09-24T12:00:00Z'},false],[{cancel_at_period_end:true,current_period_end:'2020-01-01'},false]])assert.equal(canReadMemberArticles([{...active,...patch}],now),expected);
assert.equal(canReadMemberArticles([],now),false);
assert.equal(new Set(articles.memberArticles.map(a=>a.slug)).size,39);
...
const Page=load('app/ja/my-homecourt/app/learn/[[...slug]]/page.tsx',{
 ...
 '@/lib/homecourt-billing':{HOMECOURT_BILLING_PORTAL:'https://billing.stripe.com/test'},
 './reading.module.css':{default:{}},
 'next/link':{default:props=>{const clean={...props};delete clean.prefetch;return jsx.jsx('a',clean)}},
...
records=[{...active,cancel_at_period_end:true,current_period_end:null}];let pending=await render();assert.ok(pending.includes('追加のお支払いはせず'));assert.ok(!pending.includes(secret));assert.ok(!pending.includes('href="/ja/payments"'));
...
console.log('PASS: 12 subscription cases, 39 public-safe article records (35 parent), search/category render, private-body access controls. No network or payment writes.');
