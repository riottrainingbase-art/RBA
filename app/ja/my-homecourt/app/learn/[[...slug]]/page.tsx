...
const searchableRoles = new Set(["player","parent"]);
function categoriesFor(role:string){return Array.from(new Set(memberArticles.filter(item=>item.role===role&&item.category).map(item=>item.category!)));}

export default async function Page({ params, searchParams }: { params: Promise<{ slug?: string[] }>; searchParams: Promise<{ q?: string; category?: string; audience?: string }> }) {
  const { slug } = await params;
  const query = await searchParams;
  const q = (query.q || "").trim().toLowerCase();
  const category = (query.category || "").trim();
  const audience = query.audience==="player"||query.audience==="parent" ? query.audience : "";
  const visibleFor = (role:string) => memberArticles.filter(item=>item.role===role)
    .filter(item=>audience!==role||!category||item.category===category)
    .filter(item=>audience!==role||!q||[item.title,item.summary,item.category||"",...(item.tags||[])].join(" ").toLowerCase().includes(q));
  ...
  return <main className={styles.shell}>
    ...
    <div className={styles.body}>
      ...
      {error ? <section className={styles.notice} role="alert"><h2>契約状況を確認できませんでした</h2><p>時間を置いて、もう一度このページを開いてください。</p><form method="get" action={article ? `${root}/${article.slug}` : root}><button type="submit">もう一度確認する</button></form></section>
        : pending ? <section className={styles.notice}><h2>メンバーシップの反映を確認しています</h2><p>契約の記録はありますが、閲覧期間の確認が完了していません。追加のお支払いはせず、契約状況をご確認ください。反映されない場合はお問い合わせください。</p><Link prefetch={false} href="/ja/my-homecourt/app/my">契約状況を確認する</Link><Link prefetch={false} href="/ja/contact">お問い合わせ</Link></section> : !allowed ? <section className={styles.notice}><h2>MY HOME COURTで、学びを続けよう</h2><p>記事の本文は、月額メンバーシップでお読みいただけます。お申し込み済みの方は、決済時と同じRBA IDでログインしているかご確認ください。</p><Link prefetch={false} href="/ja/my-homecourt/app/my">契約状況を確認する</Link><Link prefetch={false} href="/ja/my-homecourt/subscribe">メンバーシップの内容・申込を見る</Link></section> : null}
      {article ? allowed && body ? <article className={styles.article}>
        ...
        {Object.entries(labels).map(([role,label])=>{const visible=visibleFor(role);const searchable=searchableRoles.has(role);const activeSearch=audience===role&&(q||category);return <section className={styles.group} id={role} key={role}><h2>{label}のための読み物</h2>{searchable?<><form className={styles.search} method="get" action={root}><input type="hidden" name="audience" value={role}/><label htmlFor={`${role}-q`}>{role==="player"?"プレー・悩み・キーワードから探す":"悩み・キーワードから探す"}</label><div><input id={`${role}-q`} name="q" defaultValue={audience===role?(query.q||""):""} placeholder={role==="player"?"例：シュート、判断、1on1、守備":"例：出場、移籍、遠征、指導者"} /><select name="category" defaultValue={audience===role?category:""}><option value="">すべてのカテゴリ</option>{categoriesFor(role).map(item=><option value={item} key={item}>{item}</option>)}</select><button type="submit">探す</button></div>{activeSearch?<Link prefetch={false} href={`${root}#${role}`}>絞り込みを解除</Link>:null}</form><p className={styles.count}>{visible.length}件の{label}向けガイド</p></>:null}<div className={styles.grid}>{visible.map(item=><article className={styles.card} key={item.slug}><p className={styles.eyebrow}>{label}{item.category?` / ${item.category}`:""} / 実践ガイド</p><h3><Link prefetch={false} href={`${root}/${item.slug}`}>{item.title}</Link></h3><p>{item.summary}</p><Link prefetch={false} className={styles.read} href={`${root}/${item.slug}`}>{allowed?"記事を読む":"記事の概要を見る"} →</Link></article>)}</div>{searchable&&!visible.length?<p className={styles.empty}>該当する記事が見つかりません。別のキーワードまたはカテゴリでお試しください。</p>:null}</section>})}
      </>}
    </div>
  </main>;
}
